...
class DataPath extends Module
{
  ...
  // ### Add single step
  when (!io.ctrl.dec_stall && !io.ctrl.full_stall && io.execute)
  {
  ...
  // ### Add register reader
  regfile.io.display_addr := UInt(0,5)
...
when(io.execute) {
  ...
  // ### Add single step
  when (!io.ctrl.full_stall && io.execute)
  {
...
when(io.execute) {
  ...
  val s_if::s_dec::s_exe::s_mem::s_wb::Nil = Enum(UInt(), 5)
  val stageState = Reg(init = s_if)
  // Change the display stage
  switch (stageState) {
    is (s_if) {
      when (io.backward) { stageState := s_wb }
      when (io.forward) { stageState := s_dec }
    }
    is (s_dec) {
      when (io.backward) { stageState := s_if }
      when (io.forward) { stageState := s_exe }
    }
    is (s_exe) {
      when (io.backward) { stageState := s_dec }
      when (io.forward) { stageState := s_mem }
    }
    is (s_mem) {
      when (io.backward) { stageState := s_exe }
      when (io.forward) { stageState := s_wb }
    }
    is (s_wb) {
      when (io.backward) { stageState := s_mem }
      when (io.forward) { stageState := s_if }
    }
  }

  io.stage_if := (stageState === s_if)
  io.stage_dec := (stageState === s_dec)
  io.stage_exe := (stageState === s_exe)
  io.stage_mem := (stageState === s_mem)
  io.stage_wb := (stageState === s_wb)


  val display_num = Reg(init = UInt(0,32))
  when (io.type_1) {
    // Display registers
    val regID = UInt(io.regID_4)*UInt(16) + UInt(io.regID_3)*UInt(8) + UInt(io.regID_2)*UInt(4) + UInt(io.regID_1)*UInt(2) + UInt(io.regID_0)
    regfile.io.display_addr := regID
    display_num := regfile.io.display_data
  }
  .otherwise {
    // Display PC or instruction
    when (io.type_2) {
      // Display PC or wb_reg_wbaddr
      switch (stageState) {
        is (s_if) { display_num := if_reg_pc }
        is (s_dec) { display_num := dec_reg_pc }
        is (s_exe) { display_num := exe_reg_pc }
        is (s_mem) { display_num := mem_reg_pc }
        is (s_wb) { display_num := wb_reg_wbaddr }
      }
    } .otherwise {
      // Display instruction or wb_reg_wbdata
      switch (stageState) {
        is (s_if) { display_num := UInt(0,32) }
        is (s_dec) { display_num := dec_reg_inst }
        is (s_exe) { display_num := exe_reg_inst }
        is (s_mem) { display_num := mem_reg_inst }
        is (s_wb) { display_num := wb_reg_wbdata }
      }
    }
  }
  // Select the display part
  when (io.valueSelect) {
    io.display := display_num>>UInt(16)
  }
  .otherwise {
    io.display := display_num
  }
}
